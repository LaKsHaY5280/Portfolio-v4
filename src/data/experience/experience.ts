import { Experience } from "./types";

export const experience: Experience[] = [
  {
    company: "Astraum Digital Solutions",
    role: "Full Stack Developer",
    duration: "November 2024 – December 2024",
    responsibilities: [
      "Built a health tech platform using React.js, Node.js, and MongoDB, incorporating real-time tracking and payment integration, resulting in a 40% efficiency boost.",
      "Developed an animated, SEO-optimized website with Next.js and GSAP, increasing traffic by 60% within three months.",
      "Delivered projects 20% ahead of schedule by implementing Agile workflows, enhancing client satisfaction.",
    ],
    technologies: ["React.js", "Node.js", "MongoDB", "Next.js", "GSAP", "Agile"],
  },
  {
    company: "Byte & Kilo Co.",
    role: "Full Stack Developer",
    duration: "June 2024 – Present",
    responsibilities: [
      "Launched a digital agency platform offering web development, SEO optimization, and custom services.",
      "Secured 15+ clients in the first quarter, generating a 20% revenue growth and expanding the client base by 46%.",
      "Used Next.js, Django, and TailwindCSS to deliver scalable, performant solutions.",
    ],
    technologies: [
      "Next.js",
      "Django",
      "TailwindCSS",
      "Framer Motion",
      "AWS",
      "Shadcn",
    ],
  },
  {
    company: "Hello Study Global",
    role: "Web Developer Intern",
    duration: "June 2023 – July 2023",
    responsibilities: [
      "Engineered a platform using NEXT.js and Python for students to curate university preferences, achieving a 96.73% user satisfaction rate.",
      "Optimized the client onboarding process via A/B testing and UI/UX enhancements, leading to a 250% increase in successful client deals.",
    ],
    technologies: ["NEXT.js", "Python", "UI/UX", "A/B Testing"],
  },
];
